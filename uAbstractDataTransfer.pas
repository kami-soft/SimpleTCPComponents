...
begin
  { «десь реализован минимальный обработчик событи€ ошибки.
    „тобы не возбуждалось исключение, код ошибки обнул€ем
    и закрываем соединение, бо оно все равно уже неработоспособно.
    ≈ще раз обращаю внимание - дл€ компонентов TClient|TServerSocket
    эти две строчки - ћ»Ќ»ћјЋ№Ќџ… код, который ќЅя«јЌ быть в этом событии,
    ...
    begin
      ...
      if Readed = 0 then
        begin
          ...
          // закончилась. ”далим его и возьмем следующий.
          OutDataList.Delete(0);
          Continue;
        end;

      // отправл€ем данные в сокет
      try
      ...
      // передастс€ не об€зательно столько данных, сколько мы "заказали" -
      ...
      if Writed = -1 then
        begin
          // если ничего передать не удалось, значит буфер сокета забит нами под зав€зку,
          // пора выходить отсюда.
          // когда он освободитс€, оп€ть возникнет это событие (OnWrite)
        ...
        if Writed < Readed then
          begin
            // в сокет передалось меньше, чем мы пытались. ѕередвигаем указатель
            // потока "назад" на количество переданных байт
            Stream.Seek(Writed - Readed, {$IFDEF UseNewSeek}soCurrent{$ELSE}soFromCurrent{$ENDIF});
            // и тоже выходим - раз передалось не все, то буфер сокета забит "под зав€зку".
            break;
          end;
    end;
end;
